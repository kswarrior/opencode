...
static void handle_client(int cfd){
    ...
    if(strcmp(path,"/")==0 || strcmp(path,"/index.html")==0){
        ...
        char inner[4096];
        if(!authed){
            snprintf(inner,sizeof(inner),"<div class='btns'><a class='btn btn-primary' href=\"%s\">Sign in</a> <a class='btn btn-secondary' href=\"%s/\">My Account</a></div><p style='text-align:center;margin-top:18px;font-size:13px;color:var(--muted)'><a href=\"%s\">Secure sign-in</a> via Auth</p>", login_url, acct_url, login_url);
        } else {
            snprintf(inner,sizeof(inner),
                "<div class='user-box'><div style=\"width:48px;height:48px;margin:0 auto 10px;background:#e8f0fe;color:#1a73e8;border-radius:50%%;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700\" id=\"avatarInitial\">U</div><strong id=\"usernameDisplay\">Loading...</strong><div class=\"muted\">username</div></div>"
                "<div class='btns'><a class='btn btn-primary' href=\"%s/\">My Account</a> <a class='btn btn-secondary' href=\"%s/logout\">Logout (auth)</a></div>"
                "<a href=\"/logout\" style=\"display:block;margin-top:14px;font-size:13px;color:var(--muted);text-align:center\">Logout (main)</a>", acct_url, auth_url);
        }
        char html[16384];
        snprintf(html,sizeof(html),
"<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Main</title><script src=\"https://unpkg.com/htmx.org@1.9.12\"></script><style>:root{--blue:#1a73e8;--blue-dark:#0b57d0;--bg:#f8fbff;--card:#ffffff;--border:#e8f0fe;--text:#202124;--muted:#5f6368}*{box-sizing:border-box}body{font-family:\"Google Sans\",Roboto,system-ui,sans-serif;margin:0;background:linear-gradient(180deg,#f8fbff 0%%,#ffffff 100%%);color:var(--text);min-height:100vh;-webkit-font-smoothing:antialiased} .wrap{width:100%%;max-width:440px;margin:0 auto} .center{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px} .card{background:var(--card);border:1px solid var(--border);border-radius:24px;padding:36px 32px;box-shadow:0 1px 3px rgba(60,64,67,.12),0 8px 24px rgba(60,64,67,.08);animation:rise .5s cubic-bezier(.2,.8,.2,1);text-align:center} .logo{width:48px;height:48px;margin:0 auto 14px;background:linear-gradient(135deg,#e8f0fe 0%%,#f0f7ff 100%%);border:1px solid var(--border);border-radius:14px;display:flex;align-items:center;justify-content:center;animation:float 6s ease-in-out infinite} h1{font-size:22px;font-weight:700;margin:0;letter-spacing:-0.3px} .sub{color:var(--muted);font-size:13px;margin:6px 0 22px} .btns{display:flex;gap:12px;justify-content:center;margin-top:20px} a.btn{padding:11px 22px;border-radius:999px;font-weight:500;font-size:14px;text-decoration:none;transition:all .2s;display:inline-block;text-align:center} a.btn-primary{background:var(--blue);color:#fff;box-shadow:0 1px 3px rgba(60,64,67,.30)} a.btn-primary:hover{background:var(--blue-dark);transform:translateY(-1px)} a.btn-secondary{background:#fff;color:var(--blue);border:1px solid var(--border)} a.btn-secondary:hover{background:#f8fbff} .user-box{background:#f8f9fa;border:1px solid #e8eaed;border-radius:12px;padding:14px;margin-bottom:16px} .user-box strong{font-size:14px;color:var(--text)} .muted{color:var(--muted);font-size:12px} @keyframes float{0%%,100%%{transform:translateY(0)}50%%{transform:translateY(-3px)}} @keyframes rise{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}</style></head><body><div class='center'><div class='wrap'><div class='card'>"
